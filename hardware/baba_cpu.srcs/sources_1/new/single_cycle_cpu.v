module single_cycle_cpu (input clk,
                         input vga_clk,
                         input bram_clk,
                         input rst, // acturally it is the SW[0],defined in constraints.
                         input halt,
                         input kbd_ready,
                         input kbd_overflow,
                         input [7:0] kbd_data,
                         ...
                         output [31:0]current_pc);//new added
  ...
  pc_reg pc0(
  .clk(clk),
  .rst(rst),// to have the path to read this signal.oh holyshit,it is purely hardware,let me trace another hardware signal,to see how dose mmio work.
  .enable(~halt),
  .pc_address_in(new_pc),
  .pc_address_o(current_pc));
  
  brom instruction_rom(
  .addra(current_pc[15:2]),
  .clka(bram_clk),//这是取指令的周期时间，在这个指令执行完之前，IR都不变。
  .douta(IR));
...
//  );
  ...
  .button_state(button_state), //new added 
  .data_in(ram_data_in),
  .kbd_ready(kbd_ready),
